...
public class SpaceXHub
{
    ...
    let shipurl = URL(string: "https://api.spacexdata.com/v4/ships")
   ...
   public func getStarlinks(onCompletion :@escaping(Welcome,Error?) -> Void)
    {
     let task = session.dataTask(with: starlinkurl!) { (data, response, error) in
            ...
            DispatchQueue.main.async {
                do
                {
                    let starlinks = try JSONDecoder().decode(Welcome.self, from: data!)
                    
                    onCompletion(starlinks,error)
    ...
    public func getShips(onCompletion :@escaping(Ships,Error?) -> Void)
       {
        let task = session.dataTask(with: shipurl!) { (data, response, error) in
               
               DispatchQueue.main.async {
                   do
                   {
                       let shipss = try JSONDecoder().decode(Ships.self, from: data!)
                       
                       onCompletion(shipss,error)
